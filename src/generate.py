    ...
    for fname in os.listdir(from_path):
        ...
        if os.path.isdir(srcname):
            if not os.path.isdir(dstname):
                os.mkdir(dstname)
            generate_pages_recursive(basepath, srcname, template_path, dstname)
        else:
            name, _ = fname.split(".")
            generate_path(basepath, f"{from_path}/{name}.md", template_path, f"{dest_path}/{name}.html")

def generate_path(basepath, from_path, template_path, dest_path):
    ...
    contents_template = contents_template.replace("href=\"/", f"href=\"{basepath}")
